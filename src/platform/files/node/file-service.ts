...
// Adapted from VS Code's fileService.ts for Atlas.

import type { IExtUri } from "@core/base/resources";
import type { URI } from "@platform/common/uri/uri";
import type { IDisposable } from "@core/base/lifecycle";
import type { VSBufferReadable, VSBufferReadableBufferedStream, VSBufferReadableStream } from "@core/base/buffer";

import { hash } from "@core/base/hash";
import { Emitter } from "@core/base/event";
import { coalesce } from "@core/base/arrays";
import { ErrorNoTelemetry } from "@core/base/errors";
import { ResourceQueue } from "@core/base/async-queue";
import { ILogger } from "@platform/logger/common/logger";
import { extUri, extUriIgnorePathCase, isAbsolutePath } from "@core/base/resources";
import { CancellationToken, CancellationTokenSource } from "@core/base/cancellation";
import { Disposable, DisposableStore, dispose, toDisposable } from "@core/base/lifecycle";
import {
...
import {
  ...
  peekStream,
  transform,
} from "@core/base/stream";

import type { ICreateFileOptions, IFileContent, IFileDeleteOptions, IFileService, IFileStat, IFileStatResult, IFileStatResultWithMetadata, IFileStatWithMetadata, IFileStatWithPartialMetadata, IFileStreamContent, IFileSystemProvider, IFileSystemProviderActivationEvent, IFileSystemProviderCapabilitiesChangeEvent, IFileSystemProviderRegistrationEvent, IFileSystemProviderWithFileAtomicReadCapability, IFileSystemProviderWithFileReadStreamCapability, IFileSystemProviderWithFileReadWriteCapability, IFileSystemProviderWithOpenReadWriteCloseCapability, IFileSystemWatcher, IReadFileOptions, IReadFileStreamOptions, IResolveFileOptions, IResolveMetadataFileOptions, IStat, IWatchOptions, IWatchOptionsWithCorrelation, IWatchOptionsWithoutCorrelation, IWriteFileOptions } from "../common/files";
...
function localize(_key: string, message: string, ...args: any[]): string {
  return message.replace(/\{(\d+)\}/g, (_, index) => args[Number(index)]);
}

function mark(_name: string): void {
  // no-op: Atlas does not have performance marks
}
...
export class FileService extends Disposable implements IFileService {
  ...
  constructor(@ILogger private readonly logService: ILogger) {
    super();
  }
  ...
  * listCapabilities(): IterableIterator<{ scheme: string; capabilities: FileSystemProviderCapabilities }> {
    for (const [scheme, provider] of this.provider) {
      yield { scheme, capabilities: provider.capabilities };
    }
  }
  ...
  private async doResolveFile(resource: URI, options?: IResolveFileOptions): Promise<IFileStat> {
    ...
    return this.toFileStat(provider, resource, stat, undefined, !!resolveMetadata, (stat, siblings) => {
      ...
      const targets = resolveTo ? [resource, ...resolveTo] : [resource];
      const statResource = stat.resource.with({ query: null, fragment: null });
      for (const target of targets) {
        const targetResource = target.with({ query: null, fragment: null });
        if (providerExtUri.isEqual(statResource, targetResource) || providerExtUri.isEqualOrParent(targetResource, statResource)) {
          return true;
        }
      }
  ...
  private async toFileStat(provider: IFileSystemProvider, resource: URI, stat: IStat | { type: FileType } & Partial<IStat>, siblings: number | undefined, resolveMetadata: boolean, recurse: (stat: IFileStat, siblings?: number) => boolean): Promise<IFileStat> {
    ...
    if (fileStat.isDirectory && recurse(fileStat, siblings)) {
      try {
        ...
        const resolvedEntries = await settled(entries.map(async ([name, type]) => {
          try {
            ...
            this.logService.trace(error instanceof Error ? error.message : String(error));
  ...
  async resolveAll(toResolve: { resource: URI; options?: IResolveFileOptions }[]): Promise<IFileStatResult[]> {
    return settled(toResolve.map(async (entry) => {
      try {
  ...
  private async peekBufferForWriting(provider: IFileSystemProviderWithFileReadWriteCapability | IFileSystemProviderWithOpenReadWriteCloseCapability, bufferOrReadableOrStream: VSBuffer | VSBufferReadable | VSBufferReadableStream): Promise<VSBuffer | VSBufferReadable | VSBufferReadableStream | VSBufferReadableBufferedStream> {
    ...
    if (hasReadWriteCapability(provider) && !(bufferOrReadableOrStream instanceof VSBuffer)) {
      if (isReadableStream(bufferOrReadableOrStream)) {
        const bufferedStream = await peekStream<VSBuffer>(bufferOrReadableOrStream, 3);
  ...
  private async doCopyFolder(sourceProvider: IFileSystemProvider, sourceFolder: IFileStat, targetProvider: IFileSystemProvider, targetFolder: URI): Promise<void> {
    ...
    if (Array.isArray(sourceFolder.children)) {
      await settled(sourceFolder.children.map(async (sourceChild) => {
  ...
  watch(resource: URI, options: IWatchOptions = { recursive: false, excludes: [] }): IFileSystemWatcher | IDisposable {
    ...
    (async () => {
      try {
        ...
        this.logService.error(error instanceof Error ? error.message : String(error));
      }
    })();
  ...
  private async doWriteReadableBufferedQueued(provider: IFileSystemProviderWithOpenReadWriteCloseCapability, handle: number, readable: VSBufferReadable): Promise<void> {
    ...
    let chunk: VSBuffer | null = readable.read();
    while (chunk !== null) {
      await this.doWriteBuffer(provider, handle, chunk, chunk.byteLength, posInFile, 0);
