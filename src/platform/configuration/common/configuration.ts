...
import type { UriComponents } from "@platform/common/uri/uri";
...
import { assertNever } from "@core/base/assert";
...
export interface IConfigurationService {
   ...
   */
  updateValue: ((key: string, value: unknown) => Promise<void>) & ((key: string, value: unknown, target: ConfigurationTarget) => Promise<void>) & ((key: string, value: unknown, overrides: IConfigurationOverrides | IConfigurationUpdateOverrides) => Promise<void>) & ((key: string, value: unknown, overrides: IConfigurationOverrides | IConfigurationUpdateOverrides, target: ConfigurationTarget, options?: IConfigurationUpdateOptions) => Promise<void>);
...
function doRemoveFromValueTree(valueTree: IStringDictionary<unknown> | unknown, segments: string[]): void {
  ...
  if (Object.keys(valueTreeRecord).includes(first)) {
