import type { IDisposable } from '@renderer/common/lifecycle/dispose'

import { isEditableElement } from '@renderer/common/dom/dom'
import { Disposable } from '@renderer/common/lifecycle/dispose'

import type { ContextKeyExpression } from './parser'

import { equals } from '../helpers/utils/equals'

const KEYBINDING_CONTEXT_ATTR = 'data-ctx-id'

/**
 * Интерфейс для работы с DOM облаадющий схожими методами
 */
 ...
 * если ключ не найден в текущем контексте — запрос делегируется в родительский контекст.
 *
 * Соответствие ролям паттерна (refactoring.guru):
 * - **Handler**: `Context` (сам обрабатывает запрос и решает, передавать ли дальше)
 * - **Next Handler**: поле {@link Context#_parent | _parent}
 * - **Request**: вызов {@link Context#getValue | getValue(key)}
 *
 ...
 * Тут реализуем базовые необходимые методы, чтобы позже унаследоваться и написать доп логику (ScopedContextKeyService, OverlayContextKeyService)
 */
export abstract class AbstractContextKeyService extends Disposable implements IContextKeyService {
  ...
  // TODO: пока не будем делать обновление, т.к нужны подписки и тд
...
export class ContextKeyService extends AbstractContextKeyService implements IContextKeyService {
  ...
  private handleFocusChange() {
    function activeElementIsInput(): boolean {
      return !!document.activeElement && isEditableElement(document.activeElement)
    }
...
/**
 * Отвечает за контекст в конкретной части интерфейса (Боковая панель, поисковик и тд)
 */
...
/**
 * Временная "маска" контекста, чтобы предотвротить спам одиночными ключами/событиями
 */
...
/**
 * Удобная обертка для работы с ключами
 * Дает Type Safety и инкапсуляцияю
 */
...
/**
 * Паттерн Null-object
 * Удобная заглушка, для возврата
 * Ссылка: https://refactoring.guru/ru/introduce-null-object
 */
